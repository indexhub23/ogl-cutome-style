...
function detectActiveChange() {
    // Mutation Observer
    const btn = document.querySelector('.carousel-item')
    const options = {
        attributes: true
    }
...
function changeText(carousel, index) {
    $('.nft-text .nft-title').html(carouselData[(carousel-1)][index].name);
    $('.nft-text .nft-description').html(carouselData[(carousel-1)][index].description);
}
